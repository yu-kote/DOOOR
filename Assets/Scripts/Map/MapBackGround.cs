...
public class MapBackground : MonoBehaviour
{
    ...
    void CreateBackground(GameObject node)
    {
        ...
        if (node.GetComponent<Kyukeispace>())
        {
            ...
            BackgroundMaterialChange(1);
            use_bg = _doubleBackground;
        }
        else
        {
            BackgroundMaterialChange(0);
    ...
    /// </summary>
    void BackgroundMaterialChange(int i)
    {
        var materials = Resources.LoadAll<Material>("Prefabs/BG/Materials");
        _singleBackground.GetComponent<MeshRenderer>().material = materials[i];
        _doubleBackground.GetComponent<MeshRenderer>().material = materials[i];
    }
}
